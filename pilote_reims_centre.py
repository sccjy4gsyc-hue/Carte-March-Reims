...
def rattacher_a_iris(df: pd.DataFrame, iris_list: list[dict], col_lat: str, col_lon: str) -> pd.DataFrame:
    ...
    Robuste aux cas où df est vide ou ne contient pas les colonnes de
    coordonnées attendues (ex: aucune transaction DVF trouvée pour la
    commune) : retourne toujours un DataFrame avec les colonnes
    code_iris/nom_iris présentes, même vides, pour que le reste du
    pipeline (agreger_par_iris) ne plante jamais sur une colonne absente.
    """
    df = df.copy()
    if df.empty or col_lat not in df.columns or col_lon not in df.columns:
        df["code_iris"] = pd.Series(dtype=object)
        df["nom_iris"] = pd.Series(dtype=object)
        return df
...
def main():
    ...
    dvf_iris = rattacher_a_iris(dvf, iris_selection, "lat", "lon")
    dpe_iris = rattacher_a_iris(dpe, iris_selection, "latitude", "longitude")
    print(f"  -> {len(dvf_iris)} ventes et {len(dpe_iris)} DPE dans le périmètre du pilote")
    if dvf_iris.empty:
        print("  ATTENTION : aucune transaction DVF récupérée — vérifiez que api.cquest.org/dvf "
              "répond bien pour ce code commune (l'agrégation continuera avec DPE seul si disponible).")
