    ...
    if not iris_list:
        ...
        if not iris_selection:
            ...
            with st.spinner(f"Récupération DVF/DPE pour {len(iris_selection)} IRIS..."):
                ...
                dvf_iris = core.rattacher_a_iris(dvf, iris_selection, "lat", "lon")
                dpe_iris = core.rattacher_a_iris(dpe, iris_selection, "latitude", "longitude")
                agrege = core.agreger_par_iris(dvf_iris, dpe_iris)
                if dvf_iris.empty:
                    st.warning(
                        "Aucune transaction DVF récupérée pour cette commune (l'API "
                        "api.cquest.org/dvf n'a peut-être pas répondu comme attendu). "
                        "L'analyse continue avec les DPE seuls si disponibles."
                    )
